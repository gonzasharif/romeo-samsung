...
def convert_insight_to_metrics(prompt: str) -> Any:
    # 1. Cargar el system prompt base (busca en services/ y en root)
    template_path = os.path.join(os.path.dirname(__file__), "convert_insight.txt")
    try:
        with open(template_path, "r", encoding="utf-8") as f:
            system_prompt = f.read().strip()
    except FileNotFoundError:
        try:
            with open("convert_insight.txt", "r", encoding="utf-8") as f:
                system_prompt = f.read().strip()
        except FileNotFoundError:
            system_prompt = "Sos un evaluador simulado."
