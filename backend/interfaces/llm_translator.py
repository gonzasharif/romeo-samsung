...
def parse_json_value(value: Any) -> str:
...
def translate_llm_to_target_model(
    ...
    ocupacion = llm_data.get("occupation")
    nivel_str = llm_data.get("socioeconomic_level")
    actitud = llm_data.get("personality", [])
    ...
    return TargetModel(
        ...
        socioeconomic_level=nivel_str,
        personality=actitud if isinstance(actitud, list) else []
    )
